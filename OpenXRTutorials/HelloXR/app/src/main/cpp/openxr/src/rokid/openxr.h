...
typedef enum XrStructureType {
    ...
    XR_TYPE_PLANE_DETECTOR_GET_CHANGES_ROKID_EXT = 1000523000,
...
#define XR_EXT_rokid 1
#define XR_EXT_rokid_SPEC_VERSION         1
#define XR_EXT_rokid_EXTENSION_NAME       "XR_EXT_rokid"
typedef struct XrPlaneDetectoraGetChangesRokidExt {
    XrStructureType       type;
    void* XR_MAY_ALIAS    addPlanes;
    void* XR_MAY_ALIAS    updatePlanes;
    void* XR_MAY_ALIAS    removePlanes;
    uint32_t              addPlanesSize;
    uint32_t              updatePlanesSize;
    uint32_t              removePlanesSize;
} XrPlaneDetectoraGetChangesRokidExt;

typedef void (*PFN_xrCameraUpdateCallback)(const uint8_t* data, uint32_t size, uint32_t width, uint32_t height, uint64_t timestamp);
typedef XrResult (XRAPI_PTR *PFN_xrRecenterHeadTracker)(void);
typedef XrResult (XRAPI_PTR *PFN_xrRecenterPhonePose)(void);
typedef XrResult (XRAPI_PTR *PFN_xrGetPhonePose)(float* position, float* orientation);
typedef XrResult (XRAPI_PTR *PFN_xrGetFocalLength)(float* data);
typedef XrResult (XRAPI_PTR *PFN_xrGetPrincipalPoint)(float* data);
typedef XrResult (XRAPI_PTR *PFN_xrGetImageDimensions)(int32_t* data);
typedef XrResult (XRAPI_PTR *PFN_xrGetDistortion)(float* data);
typedef XrResult (XRAPI_PTR *PFN_xrGetHistoryCameraPhysicsPose)(uint64_t timeStamp, float* position, float* orientation);
typedef XrResult (XRAPI_PTR *PFN_xrGetCameraPhysicsPose)(uint64_t* timeStamp, float* position, float* orientation);
typedef XrResult (XRAPI_PTR *PFN_xrOpenCameraPreview)(PFN_xrCameraUpdateCallback function);
typedef XrResult (XRAPI_PTR *PFN_xrCloseCameraPreview)(void);
typedef XrResult (XRAPI_PTR *PFN_xrGetHeadTrackingStatus)(uint32_t* state);
typedef XrResult (XRAPI_PTR *PFN_xrGetSLAMQuality)(uint32_t* trackingStatus, uint32_t* imageQuality, uint32_t* kineticQuality);
typedef XrResult (XRAPI_PTR *PFN_xrRKOpenPlaneTracker)(uint32_t mode);
typedef XrResult (XRAPI_PTR *PFN_xrRKClosePlaneTracker)(void);
typedef XrResult (XRAPI_PTR *PFN_xrRKGetPlaneDetectMode)(uint32_t* mode);
typedef XrResult (XRAPI_PTR *PFN_xrRKSetPlaneDetectMode)(uint32_t mode);
typedef XrResult (XRAPI_PTR *PFN_xrRKReleaseMemory)(void* point);
typedef XrResult (XRAPI_PTR *PFN_xrRKGetUpdatePlanes)(uint32_t type, void** nochangePlanes, void** changePlanes, void** newPlanes, void** removePlanes, uint32_t* nochangePlanesSize, uint32_t* changePlanesSize, uint32_t* newPlanesSize, uint32_t* removePlanesSize);
typedef XrResult (XRAPI_PTR *PFN_xrRKGetPlanePolygon)(int64_t plane, void** polygon, uint32_t* polygonSize);
typedef XrResult (XRAPI_PTR *PFN_xrRKGetPlaneType)(int64_t plane, uint32_t* type);
typedef XrResult (XRAPI_PTR *PFN_xrRKGetPlaneCenterPose)(int64_t plane, void** pose, void** center, void** normalVector);
typedef XrResult (XRAPI_PTR *PFN_xrRKGetPlaneRectangle)(int64_t plane, void** rectangle);
typedef XrResult (XRAPI_PTR *PFN_xrRKReleasePlane)(int64_t plane);
typedef XrResult (XRAPI_PTR *PFN_xrRKCreateArtificialPlane)(float* points, uint32_t numPoints, float width, float height);

#ifndef XR_NO_PROTOTYPES
#ifdef XR_EXTENSION_PROTOTYPES
XRAPI_ATTR XrResult XRAPI_CALL xrRecenterHeadTracker(
    void                                        );

XRAPI_ATTR XrResult XRAPI_CALL xrRecenterPhonePose(
    void                                        );

XRAPI_ATTR XrResult XRAPI_CALL xrGetPhonePose(
    float*                                      position,
    float*                                      orientation);

XRAPI_ATTR XrResult XRAPI_CALL xrGetFocalLength(
    float*                                      data);

XRAPI_ATTR XrResult XRAPI_CALL xrGetPrincipalPoint(
    float*                                      data);

XRAPI_ATTR XrResult XRAPI_CALL xrGetImageDimensions(
    int32_t*                                    data);

XRAPI_ATTR XrResult XRAPI_CALL xrGetDistortion(
    float*                                      data);

XRAPI_ATTR XrResult XRAPI_CALL xrGetHistoryCameraPhysicsPose(
    uint64_t                                    timeStamp,
    float*                                      position,
    float*                                      orientation);

XRAPI_ATTR XrResult XRAPI_CALL xrGetCameraPhysicsPose(
    uint64_t*                                   timeStamp,
    float*                                      position,
    float*                                      orientation);

XRAPI_ATTR XrResult XRAPI_CALL xrOpenCameraPreview(
    PFN_xrCameraUpdateCallback                  function);

XRAPI_ATTR XrResult XRAPI_CALL xrCloseCameraPreview(
    void                                        );

XRAPI_ATTR XrResult XRAPI_CALL xrGetHeadTrackingStatus(
    uint32_t*                                   state);

XRAPI_ATTR XrResult XRAPI_CALL xrGetSLAMQuality(
    uint32_t*                                   trackingStatus,
    uint32_t*                                   imageQuality,
    uint32_t*                                   kineticQuality);

XRAPI_ATTR XrResult XRAPI_CALL xrRKOpenPlaneTracker(
    uint32_t                                    mode);

XRAPI_ATTR XrResult XRAPI_CALL xrRKClosePlaneTracker(
    void                                        );

XRAPI_ATTR XrResult XRAPI_CALL xrRKGetPlaneDetectMode(
    uint32_t*                                   mode);

XRAPI_ATTR XrResult XRAPI_CALL xrRKSetPlaneDetectMode(
    uint32_t                                    mode);

XRAPI_ATTR XrResult XRAPI_CALL xrRKReleaseMemory(
    void* XR_MAY_ALIAS                          point);

XRAPI_ATTR XrResult XRAPI_CALL xrRKGetUpdatePlanes(
    uint32_t                                    type,
    void* XR_MAY_ALIAS*                         nochangePlanes,
    void* XR_MAY_ALIAS*                         changePlanes,
    void* XR_MAY_ALIAS*                         newPlanes,
    void* XR_MAY_ALIAS*                         removePlanes,
    uint32_t*                                   nochangePlanesSize,
    uint32_t*                                   changePlanesSize,
    uint32_t*                                   newPlanesSize,
    uint32_t*                                   removePlanesSize);

XRAPI_ATTR XrResult XRAPI_CALL xrRKGetPlanePolygon(
    int64_t                                     plane,
    void* XR_MAY_ALIAS*                         polygon,
    uint32_t*                                   polygonSize);

XRAPI_ATTR XrResult XRAPI_CALL xrRKGetPlaneType(
    int64_t                                     plane,
    uint32_t*                                   type);

XRAPI_ATTR XrResult XRAPI_CALL xrRKGetPlaneCenterPose(
    int64_t                                     plane,
    void* XR_MAY_ALIAS*                         pose,
    void* XR_MAY_ALIAS*                         center,
    void* XR_MAY_ALIAS*                         normalVector);

XRAPI_ATTR XrResult XRAPI_CALL xrRKGetPlaneRectangle(
    int64_t                                     plane,
    void* XR_MAY_ALIAS*                         rectangle);

XRAPI_ATTR XrResult XRAPI_CALL xrRKReleasePlane(
    int64_t                                     plane);

XRAPI_ATTR XrResult XRAPI_CALL xrRKCreateArtificialPlane(
    float*                                      points,
    uint32_t                                    numPoints,
    float                                       width,
    float                                       height);
